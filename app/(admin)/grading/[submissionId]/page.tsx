      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Nộp bởi <strong>{profile?.full_name}</strong> · {formatDateTime(submission.created_at)}
          </p>
        </div>
        <Badge className={`text-sm ${submission.status === "graded" ? "bg-green-100 text-green-800" : "bg-orange-100 text-orange-800"}`}>
        ...
        <div className="space-y-4">
          ...
          {submission.reflection && (
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-gray-600 italic">{submission.reflection}</p>
              </CardContent>
            </Card>
          )}
        </div>
